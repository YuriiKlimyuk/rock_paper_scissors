...
def get_user_selection(actions=ACTIONS):
...
def get_computer_selection(actions=ACTIONS):
    selection = random.randint(0, len(actions) - 1)
    action = actions[selection]
    return action


def get_determine_winner(user_action, computer_action, victories=VICTORIES):
    ...
    while True:
        try:
            ...
            determine_winner = get_determine_winner(
                user_selection, computer_selection, VICTORIES
            )
